...
const convertSentenceToCaseFormats = (sentence) => {
    // 小驼峰
    const camelCase = sentence.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w)/g, (word, index) => {
        return index === 0 ? word.toLowerCase() : word.toUpperCase();
    }).replace(/\s+/g, '');

    // 大驼峰
    const pascalCase = sentence.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w)/g, (word) => {
    ...
    // 返回结果数组
    return [camelCase, pascalCase, kebabCase, trainCase, snakeCase, screamSnakeCase, sentence];
}
